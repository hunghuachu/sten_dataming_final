...
class STEN:
    ...
    def fit(self, x):
        ...
        self.network = LSTMNetwork(input_dim=self.n_features, out_dim=self.num_rank,
                                  hidden_dim=self.hidden_dim, emb_dim=512, n_emb=128, num_gru=self.num_rank)
        self.Embnetwork = LSTMNetworkEmb(input_dim=self.n_features, out_dim=self.num_rank,
        ...
        for i in range(self.epochs):
            ...
            with tqdm(total=len(train_loader),  desc=f'epoch {i+1:3d}/{self.epochs}', ncols=150,miniters=500) as pbar:# 或每 50 個 batch 更新一次
...
class InterLoss(torch.nn.Module):
    ...
    def forward(self, y_pred, y_true):
        """
        Params:
            y_pred: [batch, num_rank]
            y_true: [batch, num_rank]
        """
        preds_smax = F.softmax(y_pred, dim=1)
        true_smax = F.softmax(y_true, dim=1)

        # JS divergence components
        m = 0.5 * (preds_smax + true_smax)

        js1 = F.kl_div(
            F.log_softmax(preds_smax, dim=1),
            m,
            reduction='none'
        ).sum(dim=1)

        js2 = F.kl_div(
            F.log_softmax(true_smax, dim=1),
            m,
            reduction='none'
        ).sum(dim=1)
...
class LSTMNet(torch.nn.Module):
    ...
    def forward(self, x1, x2):
        output1, (hn1, cn1) = self.lstm(x1)
        output2, (hn2, cn2) = self.lstm(x2)
        return output1, hn1, output2, hn2   # hn (最後一層 hidden state)


class LSTMNetwork(torch.nn.Module):
    def __init__(self, input_dim, out_dim=10, hidden_dim=30, emb_dim=20,
                 n_emb=10, num_gru=10):
        super(LSTMNetwork, self).__init__()
        self.num_gru = num_gru
        self.lstm_net = LSTMNet(input_dim, hidden_dim)
    ...
    def forward(self, input1, input2):
        ...
        for i in range(self.num_gru):
            out1, hn1, out2, hn2 = self.lstm_net(input1[:, i, :, :],
                                                 input2[:, i, :, :])

            emb1 = hn1[0, :]   # 取最後一層的 hidden state
            emb2 = hn2[0, :]
...
class LSTMNetEmb(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim=20, layers=1):
        super(LSTMNetEmb, self).__init__()
        self.lstm = torch.nn.LSTM(input_dim, hidden_size=hidden_dim,
...
class LSTMNetworkEmb(torch.nn.Module):
    def __init__(self, input_dim, out_dim=10, hidden_dim=20,
                 emb_dim=20, num_gru=10):
        super(LSTMNetworkEmb, self).__init__()
        self.num_gru = num_gru
        self.lstm_net = LSTMNetEmb(input_dim, hidden_dim)
