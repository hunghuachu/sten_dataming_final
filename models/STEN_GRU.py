...
class STEN:
    ...
    def fit(self, x):
        ...
        self.network = GRUNetwork(
            ...
            out_dim=self.num_rank,
            hidden_dim=self.hidden_dim,
            emb_dim=512,
            n_emb=128,
            num_gru=self.num_rank
        )

        self.Embnetwork = GRUNetworkEmb(
            input_dim=self.n_features,
            out_dim=self.num_rank,
            hidden_dim=self.hidden_dim,
            emb_dim=512,
        ...
        for i in range(self.epochs):
            ...
            with tqdm(total=len(train_loader), desc=f'epoch {i+1:3d}/{self.epochs}', ncols=150) as pbar:
                for idx, (batch_data1, batch_data2, batch_label) in enumerate(train_loader):
                    ...
                    # --- shuffle index extraction --- #
                    seq_batch_data_lst = []
                    seq_batch_data_lst_random = []

                    for n in range(self.batch_size):
                        shuffle_label = batch_label[n] - 1
                        seq_batch_data_lst.append(seq_batch_data[n, shuffle_label, :, :])
                        seq_batch_data_lst_random.append(seq_batch_data_random[n, shuffle_label, :, :])

                    seq_batch_shuffle_data = torch.stack(seq_batch_data_lst).float().to(self.device)
                    seq_batch_shuffle_data_random = torch.stack(seq_batch_data_lst_random).float().to(self.device)
                    ...
                    xy_dis = (F.normalize(pred_dis, p=1, dim=1) *
                            F.normalize(pred_dis_random, p=1, dim=1)).sum(dim=1)

                    pred_target = self.Embnetwork(seq_batch_data).detach()
                    pred_target_random = self.Embnetwork(seq_batch_data_random).detach()

                    x_y_dis = (F.normalize(pred_target, p=1, dim=1) *
                            F.normalize(pred_target_random, p=1, dim=1)).sum(dim=1)
            ...
            # --- epoch finished --- #
            train_loss_avg = float(np.mean(train_loss_lst))
            epoch_loss_list.append(train_loss_avg)

            best_loss = min(best_loss, train_loss_avg)

            t = time.time() - t1
            print(f'epoch {i+1:3d}/{self.epochs}: loss={train_loss_avg:.6f} | time={t:.1f}s | steps={steps}')

            for name, param in self.network.named_parameters():
                if param.grad is not None and (torch.isnan(param.grad).any() or torch.isinf(param.grad).any()):
                    print(f"Gradient contains NaN or inf values for: {name}")

        # ---- Return loss and best ---- #
        return epoch_loss_list, best_loss
...
class GRUNet(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim=20, layers=1):
        ...
        self.gru = torch.nn.GRU(input_dim, hidden_size=hidden_dim, 
